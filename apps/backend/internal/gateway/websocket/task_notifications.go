...
func RegisterTaskNotifications(ctx context.Context, eventBus bus.EventBus, hub *Hub, log *logger.Logger) *TaskEventBroadcaster {
	...
	b.subscribe(eventBus, events.TaskObjectiveRunUpdated, ws.ActionTaskObjectiveRunUpdated)
	b.subscribe(eventBus, events.TaskObjectivePublished, ws.ActionTaskObjectivePublished)
	b.subscribe(eventBus, events.TaskObjectiveCleared, ws.ActionTaskObjectiveCleared)
